      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-8 max-w-sm w-full text-center animate-fade-in-up">
            <h3 className="text-xl font-bold mb-4 text-slate-900">Delete Landing Page?</h3>
            <p className="mb-6 text-slate-600">Are you sure you want to delete this landing page? This action cannot be undone.</p>
            {error && <div className="text-red-500 text-sm mb-2">{error}</div>}
            <div className="flex gap-4 justify-center">
              <button
                className="px-6 py-2 bg-red-600 text-white rounded-lg font-semibold shadow hover:bg-red-700 transition disabled:opacity-60"
                onClick={() => handleDelete(deleteId)}
                disabled={deleting}
              >{deleting ? "Deleting..." : "Delete"}</button>
              <button
                className="px-6 py-2 bg-slate-200 text-slate-700 rounded-lg font-semibold shadow hover:bg-slate-300 transition"
